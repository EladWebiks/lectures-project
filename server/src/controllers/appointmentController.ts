...
export const getAppointments = async (
    ...
    try {
      const user = await User.findById((req as any).user.id).populate("appointments", process.env.KEYVALUESTOREMOVEFROMAPPOINTMENT)
      if (!user) {
        res.status(400).json({
          success: false,
      ...
      if (user.isAdmin) {
        const appointments = await Appointment.find({}).populate("user", process.env.KEYVALUESTOREMOVEFROMUSER);
        res.status(200).json({
          success: true,
